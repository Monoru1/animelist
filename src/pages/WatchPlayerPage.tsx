...
async function fetchWatchPayload(animeId: string) {
  const [{ data: anime, error }, { data: episodes }] = await Promise.all([
  ...
  return { anime: anime as WatchAnime, episodes: (episodes ?? []) as unknown as Episode[] }
}
...
export function WatchPlayerPage() {
  ...
  const selectedSrc = allSources.find((s) => s.id === selSrcId) ?? allSources.find((s) => s.is_default) ?? allSources[0] ?? null
  ...
  useEffect(() => {
    ...
    if (isVid && videoRef.current) {
      saveTimer.current = setInterval(() => {
        void saveProgress(anime.id, currentEp.id, Math.round(videoRef.current?.currentTime ?? 0))
      }, 30_000)
    }
  ...
  async function reportSource() {
    ...
    await supabase.from('source_reports').insert({
      ...
      episode_id: currentEp?.id ?? null, reported_by: authData.user?.id ?? null,
        ...
        <div className="watch-main">
          ...
          <div className="watch-frame">
            {hasSources && selectedSrc && !iframeErr ? (
              isVid ? (
                ...
                <iframe
                  key={selectedSrc.id} src={selectedSrc.source_url}
                  title={`${anime.title} EP${currentEp?.episode_number ?? epIdx + 1}`}
                  allow="autoplay; fullscreen; picture-in-picture; encrypted-media"
                  allowFullScreen className="watch-iframe"
                  onError={() => setIframeErr(true)}
                />
              )
            ) : (
          ...
          <div className="watch-controls surface-panel">
            ...
            <div className="watch-nav-btns">
              ...
              <Link className="secondary-btn" to={ROUTES.ANIME_DETAIL(anime.id)}>Fiche anime</Link>
              {hasSources && selectedSrc && !iframeErr ? (
                <button className="secondary-btn watch-report-btn" type="button" onClick={() => void reportSource()} disabled={reported}>
          ...
          {/* Chat mobile — sous les contrôles */}
          {chatOpen && userId ? (
            <WatchChatPanel
              animeId={animeId}
              ...
              className="watch-chat-mobile"
            />
          ) : null}
        </div>

        {/* ── Sidebar droite : épisodes + chat desktop ─────────── */}
        <div className="watch-right-col">
          {/* Chat desktop */}
          {userId ? (
            <WatchChatPanel
              animeId={animeId}
          ...
          <aside className="watch-sidebar surface-panel">
            ...
            <div className="episode-list">
              {episodes.map((ep, idx) => {
                ...
                const hasEpSrc = epSrc.length > 0
                return (
                  <button
                    key={ep.id} type="button"
                  ...
                  >
                    <div className="episode-thumb-wrap">
                      ...
                      {!hasEpSrc ? <span className="episode-no-src-badge">—</span> : null}
                    </div>
